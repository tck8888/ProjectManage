...
public class PresiftingUpdateFragment extends BaseFragment1 implements View.OnClickListener {
    ...
    public void submitData() {
        ...
        HttpRequest.getInstance().post(ProjectManageHttpUrlList.PROJECT_JOB_SUBJECT_FILTER_URL, map, tag, new OnServerCallBack<HttpResult<Object>, Object>() {
            @Override
    ...
    private boolean checkData(String operation_date, String prescreen_count, String meet_count, String job_time) {
        ...
        if (TextUtils.isEmpty(prescreen_count)) {
            Toast.makeText(getContext(), "请输入患者数", Toast.LENGTH_SHORT).show();
            return true;
        }
        if (TextUtils.isEmpty(meet_count)) {
            Toast.makeText(getContext(), "请输入初步符合患者数", Toast.LENGTH_SHORT).show();
            return true;
        }
